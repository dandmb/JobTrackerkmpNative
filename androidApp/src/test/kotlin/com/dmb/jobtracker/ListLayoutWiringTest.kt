...
class ListLayoutWiringTest {
    ...
    fun emptyState_isCheckedBeforeTheNoResultMessage() {
        ...
        assertTrue(whenBlock.indexOf("state.offers.isEmpty() -> EmptyOffersMessage()") in 0 until whenBlock.indexOf("Aucun résultat pour"),
    ...
    fun emptyState_multiLineTextsAreCentered() {
        ...
        assertTrue(Regex("""Text\("Aucune candidature",[^)]*textAlign = TextAlign.Center""").containsMatchIn(empty))
        assertTrue(empty.substringAfter("Ajoute ta première candidature").contains("textAlign = TextAlign.Center"),

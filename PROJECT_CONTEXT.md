...
**CI (tests uniquement)** : `.github/workflows/ci.yml`, workflow « CI ». **Le CD Android staging est un workflow séparé (voir plus bas) ; `ci.yml` lui-même n'a ni signature, ni secret, ni déploiement.** Détails, versions vérifiées et procédure de branch protection : `rapport-ci-setup.md`.
...
**CD Android staging** : `.github/workflows/cd-android-staging.yml` (détails : `rapport-cd-android-staging.md`). Push sur `staging` (merge de `dev`) + `workflow_dispatch` → APK **signé** `com.dmb.joblog.staging` → Firebase App Distribution, groupe `beta-testeurs`. Job `deploy` (`ubuntu-24.04`, 30 min, `environment: staging`, `permissions: contents: read`, `concurrency` sans annulation).
- **Gradle (`androidApp/build.gradle.kts`)** : build type `staging` (`initWith(release)`, suffixe `.staging`, `matchingFallbacks += "release"`) ; signature lue dans `STAGING_KEYSTORE_PATH` / `STAGING_KEYSTORE_PASSWORD` / `STAGING_KEY_ALIAS` (mot de passe de clé = celui du keystore) — **si une variable manque, aucun signingConfig n'est créé et l'APK sort `androidApp-staging-unsigned.apk`, sans erreur Gradle** (local et CI de tests inchangés) ; `versionCode` = `VERSION_CODE` (défaut 1, la CD passe `github.run_number`) ; `BuildConfig.IS_STAGING` (`true` uniquement en staging, `buildFeatures.buildConfig = true`) ; nom d'app « JobLog STG » via `src/staging/res/values/` **et `values-fr/`** (sinon `values-fr/app_name` de `main` gagnerait en français). « À propos » suffixe le libellé de version par « · STG » en staging (`AppVersion.kt`).
- **Sécurité du workflow** : secrets exposés seulement aux steps qui les utilisent ; seul le chemin du keystore passe par `GITHUB_ENV` ; release notes = `git log` écrit par le shell dans `$GITHUB_WORKSPACE/release-notes.txt` (l'action est Docker : elle ne voit que le workspace) puis `releaseNotesFile: release-notes.txt` (aucun message de commit dans un `${{ }}` de `run:`) ; `apksigner verify` fait échouer le job si l'APK n'est pas signé ; l'action `wzieba/Firebase-Distribution-Github-Action` est épinglée par SHA (v1.7.1) ; step `Cleanup` en `if: always()` : supprime keystore, `release-notes.txt` et `service_credentials_content.json`.
- **Piège** : `AboutScreenWiringTest.fact_noAnalyticsAdsOrNetworkLibraryIsDeclared` cherche des mots (dont « firebase ») dans **tout le texte** du catalogue et des `build.gradle.kts`, commentaires compris : n'y écrire ni « firebase » ni « analytics ». Le mot est autorisé dans les workflows (non scannés) ; l'app elle-même n'embarque aucune dépendance Firebase.
- **Réglages manuels (hors repo)** : environment GitHub `staging` avec les 5 secrets (FIREBASE_APP_ID, FIREBASE_SERVICE_ACCOUNT_JSON, STAGING_KEYSTORE_BASE64, STAGING_KEYSTORE_PASSWORD, STAGING_KEY_ALIAS) ; restreindre les branches de déploiement de cet environment à `staging` (sinon un `workflow_dispatch` depuis une autre branche déploie son code) ; groupe de testeurs `beta-testeurs` créé dans Firebase.

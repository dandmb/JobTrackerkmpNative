 ...
 *  6. Un salaire MAX sans MIN est refusé à l'enregistrement, avec un message ([SALARY_MIN_REQUIRED_MESSAGE]) : l'écran
...
object JobOfferFormLogic {
    ...
    /** Message affiché quand le salaire max est rempli sans salaire min (règle 6). */
    const val SALARY_MIN_REQUIRED_MESSAGE = "Renseigne aussi le salaire minimum, ou laisse les deux champs vides."
     ...
     */
    fun validate(title: String, company: String, salaryMin: String, salaryMax: String): FormValidation = when {
        digitsOf(salaryMin).isEmpty() && digitsOf(salaryMax).isNotEmpty() ->
            FormValidation.InvalidSalary(SALARY_MIN_REQUIRED_MESSAGE)
